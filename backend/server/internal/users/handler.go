...
func (h *handler) Register(router *mux.Router) {
	router.HandleFunc("/register", middleware.ErrorMiddleware(h.RegisterUser)).Methods(http.MethodPost, http.MethodOptions)
	router.HandleFunc("/login", middleware.ErrorMiddleware(h.AuthUser)).Methods(http.MethodPost, http.MethodOptions)
	router.HandleFunc("/chats/create", middleware.ErrorMiddleware(middleware.AuthMiddleware(h.CreateChat))).Methods(http.MethodPost, http.MethodOptions)
	router.HandleFunc("/chats/get", middleware.ErrorMiddleware(middleware.AuthMiddleware(h.GetUserChats))).Methods(http.MethodGet, http.MethodOptions)
	router.HandleFunc("/users", middleware.ErrorMiddleware(middleware.AuthMiddleware(h.GetAllUsernames))).Methods(http.MethodGet, http.MethodOptions)
	router.HandleFunc("/chats/{chatId}", middleware.ErrorMiddleware(middleware.AuthMiddleware(h.DeleteChat))).Methods(http.MethodDelete, http.MethodOptions)
	router.HandleFunc("/chats/{chatId}/{username}", middleware.ErrorMiddleware(middleware.AuthMiddleware(h.RemoveUser))).Methods(http.MethodDelete, http.MethodOptions)
	router.HandleFunc("/chats/{chatId}/{username}", middleware.ErrorMiddleware(middleware.AuthMiddleware(h.AddUser))).Methods(http.MethodPost, http.MethodOptions)
}
